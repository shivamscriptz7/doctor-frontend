...
//                 </div>
//                 <p className="text-xs text-slate-400">Required columns: name, type</p>
//               </div>
//             </div>
//           )}
...
//       ? { name: medicine.name, type: medicine.category}
//       : EMPTY_FORM
//     );
...
//   const exportToExcel = () => {
//     const headers = ['Sr.No', 'ID', 'Name', 'Type', 'Strength'

//     ];
//     const csv = [headers.join(','), ...filteredMedicines.map((m, i) =>
//       `"${(currentPage - 1) * itemsPerPage + i + 1}","${m.id}","${m.name}","${m.category}"`
//     )].join('\n');
//     const link = document.createElement('a');
...
//             { label: 'Total Value',     value: '', color: 'text-emerald-600' },
...
//             <input
//               type="text" placeholder="Search by medicine name, ID..."
//               value={searchTerm}
...
//                 <tr>
//                   {['Sr. No.', 'ID', 'Name', 'Type','Actions'].map((h, i) => (
...
//                       </div>
//                     </td>
//                     <td className="px-6 py-4  text-slate-800">{medicine.name}</td>
//                     <td className="px-6 py-4">
...
const getHospitalId  = () => getUserFromStorage()?.hospitalId ?? null;
const getCreatedById = () => getUserFromStorage()?.id         ?? null;
...
function TabSwitcher({ active, onChange }) {
  return (
    <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl w-fit">
      {[
        { key: 'single', icon: <Pill className="w-4 h-4" />,   label: 'Single' },
        { key: 'bulk',   icon: <Upload className="w-4 h-4" />, label: 'Bulk Upload' },
      ].map(({ key, icon, label }) => (
        <button
          key={key}
          type="button"
          ...
          className={`px-4 py-2 rounded-lg text-sm font-semibold transition-all flex items-center gap-2 ${
            active === key ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'
          }`}
        >
          {icon} {label}
        </button>
...
function BulkUploadPanel({ onBulkUpload, isSubmitting, uploadSummary, onReset }) {
  ...
  const totalPreviewPages = Math.ceil(previewRows.length / PREVIEW_PAGE_SZ);

  const parseFile = (file) => {
    setParseError(''); setPreviewHeaders([]); setPreviewRows([]);
    setTotalRowCount(0); setPreviewPage(1);
    ...
    reader.onload = (e) => {
      try {
        ...
        const ws   = wb.Sheets[wb.SheetNames[0]];
        const data = XLSX.utils.sheet_to_json(ws, { header: 1, defval: '' });
        if (data.length < 2) { setParseError('File empty hai ya sirf header row hai.'); return; }

        const lowerHeaders = data[0].map(String).map(h => h.trim().toLowerCase());
        if (!lowerHeaders.includes('name') || !lowerHeaders.includes('type')) {
          setParseError('Required columns missing: "name" and "type" must be present.');
          return;
        }
        const displayHeaders = data[0].map(String).map(h => h.trim());
        const rawRows        = data.slice(1).filter(r => r.some(c => String(c).trim() !== ''));
        setPreviewHeaders(displayHeaders);
        setPreviewRows(rawRows.slice(0, PREVIEW_LIMIT));
        setTotalRowCount(rawRows.length);
        setSelectedFile(file);
      } catch (err) { setParseError('File parse nahi ho saki: ' + err.message); }
    };
    reader.readAsBinaryString(file);
  };
  ...
  const handleRemove = () => {
    setSelectedFile(null); setPreviewHeaders([]); setPreviewRows([]);
    setTotalRowCount(0); setPreviewPage(1); setParseError(''); onReset?.();
  };

  // Inject hospital_id & created_by from localStorage into FormData
  const handleUpload = () => {
    if (!selectedFile) return;
    const hospitalId = getHospitalId();
    const createdBy  = getCreatedById();
    if (!hospitalId || !createdBy) { setParseError('User session not found. Please login again.'); return; }
    const fd = new FormData();
    fd.append('file',        selectedFile);
  ...
  const getPagedRows  = () => previewRows.slice((previewPage - 1) * PREVIEW_PAGE_SZ, previewPage * PREVIEW_PAGE_SZ);
  const getPageNums   = () => {
    const pages = [];
    if (totalPreviewPages <= 7) { for (let i = 1; i <= totalPreviewPages; i++) pages.push(i); }
    else if (previewPage <= 4)  { pages.push(1,2,3,4,5,'...',totalPreviewPages); }
    else if (previewPage >= totalPreviewPages - 3) { pages.push(1,'...',totalPreviewPages-4,totalPreviewPages-3,totalPreviewPages-2,totalPreviewPages-1,totalPreviewPages); }
    else { pages.push(1,'...',previewPage-1,previewPage,previewPage+1,'...',totalPreviewPages); }
    return pages;
  };
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between p-3 bg-emerald-50 rounded-xl border border-emerald-100">
        <div className="flex items-center gap-2 text-sm text-emerald-700">
          <FileText className="w-4 h-4" />
          <span className="font-medium">Need a template?</span>
          <span className="text-emerald-600">Only name &amp; type columns needed</span>
        </div>
        <button
          type="button"
          onClick={() => {
      ...
      <div className="flex items-start gap-2 p-3 bg-blue-50 border border-blue-100 rounded-xl text-xs text-blue-700">
        <AlertCircle className="w-4 h-4 mt-0.5 flex-shrink-0" />
        <span>
          <strong>hospital_id</strong> and <strong>created_by</strong> are automatically taken from your session and saved for every row — no need to include them in the file.
        </span>
      </div>
        ...
        <div className="space-y-5">
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-emerald-500 to-teal-500 p-px">
            <div className="rounded-2xl bg-gradient-to-r from-emerald-50 to-teal-50 px-5 py-4 flex items-center gap-4">
              ...
              <div>
                <p className="font-bold text-emerald-800 text-base">Upload Successful!</p>
                <p className="text-sm text-emerald-600 mt-0.5">{uploadSummary.processed} medicines added in {uploadSummary.timeTaken}</p>
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: 'Total Rows', value: uploadSummary.totalRows, icon: <Table2 className="w-4 h-4" />,       bg: 'bg-slate-50',   border: 'border-slate-200',  iconBg: 'bg-slate-100',   iconColor: 'text-slate-500',   valueColor: 'text-slate-800',   labelColor: 'text-slate-500' },
              { label: 'Processed',  value: uploadSummary.processed, icon: <CheckCircle2 className="w-4 h-4" />, bg: 'bg-emerald-50', border: 'border-emerald-200', iconBg: 'bg-emerald-100', iconColor: 'text-emerald-600', valueColor: 'text-emerald-700', labelColor: 'text-emerald-600' },
              { label: 'Skipped',    value: uploadSummary.skipped,   icon: <AlertCircle className="w-4 h-4" />,  bg: uploadSummary.skipped > 0 ? 'bg-amber-50' : 'bg-slate-50', border: uploadSummary.skipped > 0 ? 'border-amber-200' : 'border-slate-200', iconBg: uploadSummary.skipped > 0 ? 'bg-amber-100' : 'bg-slate-100', iconColor: uploadSummary.skipped > 0 ? 'text-amber-600' : 'text-slate-400', valueColor: uploadSummary.skipped > 0 ? 'text-amber-700' : 'text-slate-500', labelColor: uploadSummary.skipped > 0 ? 'text-amber-600' : 'text-slate-400' },
              { label: 'Time Taken', value: uploadSummary.timeTaken, icon: <Loader2 className="w-4 h-4" />,      bg: 'bg-blue-50',    border: 'border-blue-200',   iconBg: 'bg-blue-100',    iconColor: 'text-blue-500',    valueColor: 'text-blue-700',    labelColor: 'text-blue-500' },
            ].map(({ label, value, icon, bg, border, iconBg, iconColor, valueColor, labelColor }) => (
              <div key={label} className={`${bg} border ${border} rounded-xl p-3 sm:p-4 flex flex-col gap-2`}>
        ...
        <>
          ...
          {!selectedFile && (
            ...
            >
              <input ref={fileInputRef} type="file" accept=".csv,.xlsx,.xls" className="hidden" onChange={handleFileSelect} />
              <div className="flex flex-col items-center gap-3">
                <div className={`w-14 h-14 rounded-full flex items-center justify-center ${dragOver ? 'bg-emerald-100' : 'bg-slate-100'}`}>
                  <Upload className={`w-7 h-7 ${dragOver ? 'text-emerald-600' : 'text-slate-400'}`} />
                </div>
                <div>
                  <p className="font-semibold text-slate-700">Drag & drop your file here</p>
                ...
                <p className="text-xs text-slate-400">Required columns: <strong>name</strong>, <strong>type</strong></p>
              </div>
            </div>
          )}
          ...
          {parseError && (
            <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-100 rounded-xl text-sm text-red-700">
              <AlertCircle className="w-4 h-4 mt-0.5 flex-shrink-0" />
              <div><p className="font-semibold">Error</p><p>{parseError}</p></div>
            </div>
          )}
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-700">{selectedFile.name}</p>
                    <p className="text-xs text-slate-400">{(selectedFile.size / 1024).toFixed(1)} KB • <span className="font-medium text-slate-600">{totalRowCount.toLocaleString()} rows</span></p>
                  </div>
                </div>
                <button type="button" onClick={handleRemove} className="text-sm text-red-500 hover:text-red-700 font-medium flex items-center gap-1 transition-colors">
                  <X className="w-3.5 h-3.5" /> Remove
                </button>
              </div>
              ...
              {totalRowCount > PREVIEW_LIMIT && (
                <div className="flex items-center gap-2 p-3 bg-amber-50 rounded-xl border border-amber-200 text-sm text-amber-700">
                  <AlertCircle className="w-4 h-4 flex-shrink-0" />
                  <span>File mein <strong>{totalRowCount.toLocaleString()}</strong> rows hain — sirf pehle <strong>{PREVIEW_LIMIT}</strong> preview mein hain. Saare upload honge.</span>
                </div>
              )}
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-full text-xs font-semibold">
                  {Math.min(totalRowCount, PREVIEW_LIMIT).toLocaleString()} of {totalRowCount.toLocaleString()} rows
                </span>
              </div>

              {/* Table with auto-injected columns shown */}
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead className="bg-gradient-to-r from-emerald-50 to-teal-50">
                      <tr>
                        <th className="px-3 py-2.5 text-left font-semibold text-slate-500 border-b border-emerald-100">#</th>
                        {previewHeaders.map(h => (
                          <th key={h} className="px-3 py-2.5 text-left font-semibold text-slate-600 border-b border-emerald-100 capitalize whitespace-nowrap">{h}</th>
                        ))}
                        <th className="px-3 py-2.5 text-left font-semibold text-blue-500 border-b border-emerald-100 whitespace-nowrap">
                          hospital_id <span className="font-normal text-blue-400">(auto)</span>
                        </th>
                        <th className="px-3 py-2.5 text-left font-semibold text-blue-500 border-b border-emerald-100 whitespace-nowrap">
                          created_by <span className="font-normal text-blue-400">(auto)</span>
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {getPagedRows().map((row, i) => (
                        <tr key={i} className="hover:bg-emerald-50/50 transition-colors">
                          <td className="px-3 py-2 text-slate-400 font-medium">{(previewPage - 1) * PREVIEW_PAGE_SZ + i + 1}</td>
                          {previewHeaders.map((_, j) => (
                            <td key={j} className="px-3 py-2 text-slate-700 whitespace-nowrap">{String(row[j] ?? '')}</td>
                          ))}
                          <td className="px-3 py-2 text-blue-500 font-medium">{getHospitalId() ?? '—'}</td>
                          <td className="px-3 py-2 text-blue-500 font-medium">{getCreatedById() ?? '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {totalPreviewPages > 1 && (
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500">
                    {(previewPage - 1) * PREVIEW_PAGE_SZ + 1}–{Math.min(previewPage * PREVIEW_PAGE_SZ, previewRows.length)} of {previewRows.length}
                  </span>
                  <div className="flex items-center gap-1">
                    <button onClick={() => setPreviewPage(p => p - 1)} disabled={previewPage === 1}
                      className="px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white hover:bg-emerald-50 disabled:opacity-40 disabled:cursor-not-allowed font-semibold text-slate-600 transition-all">‹</button>
                    {getPageNums().map((p, idx) =>
                      p === '...' ? <span key={`e${idx}`} className="px-1 text-slate-400">…</span> : (
                        <button key={p} onClick={() => setPreviewPage(p)}
                          className={`px-2.5 py-1.5 rounded-lg border font-semibold transition-all ${previewPage === p ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white border-transparent' : 'border-slate-200 bg-white text-slate-600 hover:bg-emerald-50'}`}>
                          {p}
                        </button>
                      )
                    )}
                    <button onClick={() => setPreviewPage(p => p + 1)} disabled={previewPage === totalPreviewPages}
                      className="px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white hover:bg-emerald-50 disabled:opacity-40 disabled:cursor-not-allowed font-semibold text-slate-600 transition-all">›</button>
                  </div>
                </div>
              )}
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Medicine Name *</label>
          <input type="text" name="name" value={formData.name} onChange={onChange} required placeholder="e.g., Paracetamol"
            className="w-full px-4 py-2.5 bg-slate-50 border-2 border-transparent rounded-xl focus:outline-none focus:border-emerald-500 focus:bg-white transition-all text-slate-700 text-sm" />
        </div>
        <div>
          <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5">Type *</label>
...
// ── Helpers ───────────────────────────────────────────────────────────────────
const getStatusColor = (s) => ({ 'in-stock': 'bg-green-100 text-green-700', 'low-stock': 'bg-amber-100 text-amber-700', 'critical': 'bg-red-100 text-red-700' }[s] ?? 'bg-slate-100 text-slate-700');
const getStockIcon   = (s) => ({ 'in-stock': <Package className="w-4 h-4" />, 'low-stock': <TrendingUp className="w-4 h-4" />, 'critical': <AlertTriangle className="w-4 h-4" /> }[s] ?? <Package className="w-4 h-4" />);
const mapMedicine    = (item) => ({ id: item.id, name: item.name, category: item.type || 'General', status: item.status || 'in-stock', stock: item.stock || 0, price: item.price || 0 });
...
export default function PharmacyPage() {
  ...
  const openModal = (type, medicine = null) => {
    setSelectedMedicine(medicine);
    setFormData(medicine && type === 'edit' ? { name: medicine.name, type: medicine.category } : EMPTY_FORM);
    if (type === 'add') setAddTab('single');
    setModal({ type });
  };
  const closeModal = () => { setModal({ type: null }); setSelectedMedicine(null); setFormData(EMPTY_FORM); setAddTab('single'); setUploadSummary(null); };

  // ── Fetch — server-side search + filter + pagination ──────────────────────
  const loadMedicines = useCallback(async (page = currentPage) => {
    setLoading(true);
    try {
      // getMedicines should accept (limit, page, search?, type?) — update your API function accordingly
  ...
  }, [currentPage, itemsPerPage, debouncedSearch, filterCategory]);

  useEffect(() => { loadMedicines(currentPage); }, [currentPage, debouncedSearch, filterCategory]);
  ...
  // ── Submit — inject hospital_id & created_by ──────────────────────────────
  const handleSubmit = async (e) => {
    e.preventDefault();
    setIsSubmitting(true);
    const payload = { name: formData.name, type: formData.type, hospital_id: getHospitalId(), created_by: getCreatedById() };
    try {
      if (modal.type === 'edit') {
        await updatePharmacyApi(selectedMedicine.id, payload);
        showToast('success', 'Updated', 'Medicine updated successfully!');
  ...
  // ── Bulk Upload ───────────────────────────────────────────────────────────
  const handleBulkUpload = async (fd) => {
    setIsSubmitting(true);
    const start = Date.now();
    try {
      const res     = await createBulkMedicineApi(fd);
      const elapsed = ((Date.now() - start) / 1000).toFixed(1) + 's';
      const d       = res?.summary || {};
      setUploadSummary({ totalRows: d.totalRows ?? '—', processed: d.processed ?? '—', skipped: d.skipped ?? 0, timeTaken: d.timeTaken ?? elapsed });
      setCurrentPage(1);
      await loadMedicines(1);
      showToast('success', 'Bulk Upload Done', 'Medicines uploaded successfully!');
    } catch (err) {
  ...
  const stats = useMemo(() => ({
    ...
    lowStock:   medicines.filter(m => m.status === 'low-stock').length,
    critical:   medicines.filter(m => m.status === 'critical').length,
    totalValue: medicines.reduce((sum, m) => sum + (m.stock ?? 0) * (m.price ?? 0), 0),
  }), [medicines, totalCount]);
  ...
  const exportToExcel = () => {
    const csv = ['Sr.No,ID,Name,Type', ...medicines.map((m, i) => `"${indexOfFirstItem+i}","${m.id}","${m.name}","${m.category}"`)].join('\n');
    const link = document.createElement('a');
    link.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8;' }));
    link.download = `pharmacy_${new Date().toISOString().split('T')[0]}.csv`;
    document.body.appendChild(link); link.click(); document.body.removeChild(link);
  ...
  const exportToPDF = () => {
    const html = `<!DOCTYPE html><html><head><title>Pharmacy Report</title><style>body{font-family:Arial,sans-serif;margin:20px}h1{color:#059669;text-align:center}table{width:100%;border-collapse:collapse;margin-top:20px}th{background:#059669;color:white;padding:12px;text-align:left}td{padding:10px;border-bottom:1px solid #ddd}</style></head><body><h1>Pharmacy Report</h1><p>Generated: ${new Date().toLocaleDateString('en-IN')} | Total: ${totalCount}</p><table><thead><tr><th>Sr.No</th><th>ID</th><th>Name</th><th>Type</th></tr></thead><tbody>${medicines.map((m, i) => `<tr><td>${indexOfFirstItem+i}</td><td>${m.id}</td><td>${m.name}</td><td>${m.category}</td></tr>`).join('')}</tbody></table></body></html>`;
    const w = window.open('', '', 'height=600,width=800');
    w.document.write(html); w.document.close(); w.focus();
    setTimeout(() => { w.print(); setShowExportMenu(false); }, 250);
  };
    ...
    <div className="p-8">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-slate-100 p-6 mb-6">
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-1 relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5" />
            <input type="text" placeholder="Search by medicine name, ID..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-12 pr-10 py-3 bg-slate-50 border-2 border-transparent rounded-xl focus:outline-none focus:border-emerald-500 focus:bg-white transition-all text-slate-700"
            />
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gradient-to-r from-emerald-50 to-teal-50 border-b-2 border-emerald-200">
                <tr>
                  {['Sr. No.', 'ID', 'Name', 'Type', 'Actions'].map(h => (
                    <th key={h} className="px-5 py-3.5 text-left text-xs font-bold text-slate-600 uppercase tracking-wider whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
                  ...
                  <tr>
                    <td colSpan={5} className="px-6 py-12 text-center">
                      ...
                      <p className="text-slate-600 font-medium">
                        {debouncedSearch ? `No medicines found for "${debouncedSearch}"` : 'No medicines found'}
                      </p>
                    </td>
                  </tr>
                )}
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1">
                  {getPageNumbers().map((pageNum, idx) =>
                    pageNum === '...' ? (
                      <span key={`e-${idx}`} className="px-3 py-2 text-slate-400">...</span>
                    ) : (
                      <button key={pageNum} onClick={() => setCurrentPage(pageNum)}
                        className={`px-4 py-2 rounded-lg font-medium transition-all ${currentPage === pageNum ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-lg' : 'bg-white text-slate-700 hover:bg-emerald-50 hover:text-emerald-600 border border-slate-200'}`}>
                        {pageNum}
      ...
      {/* Add Modal */}
      <Modal isOpen={modal.type === 'add'} onClose={closeModal} title={addTab === 'bulk' ? 'Bulk Upload Medicines' : 'Add New Medicine'} size={addTab === 'bulk' ? 'lg' : 'md'}>
        <div className="space-y-5">
          <TabSwitcher active={addTab} onChange={setAddTab} />
          {addTab === 'single' && <MedicineForm formData={formData} onChange={handleFormChange} onSubmit={handleSubmit} onCancel={closeModal} isEdit={false} isSubmitting={isSubmitting} />}
          {addTab === 'bulk'   && <BulkUploadPanel onBulkUpload={handleBulkUpload} isSubmitting={isSubmitting} uploadSummary={uploadSummary} onReset={() => setUploadSummary(null)} />}
        </div>
      </Modal>
      ...
      <Modal isOpen={modal.type === 'view'} onClose={closeModal} title="Medicine Details" size="md">
        {selectedMedicine && (
          <div className="space-y-6">
            ...
            <div className="flex gap-3 pt-4 border-t border-slate-200">
              <button onClick={() => { closeModal(); openModal('edit', selectedMedicine); }}
                className="flex-1 py-2.5 rounded-xl bg-emerald-50 text-emerald-700 font-semibold hover:bg-emerald-100 transition-colors">Edit Medicine</button>
              <button onClick={closeModal}
                className="flex-1 py-2.5 rounded-xl bg-slate-100 text-slate-700 font-semibold hover:bg-slate-200 transition-colors">Close</button>
            </div>
          </div>
        )}
      </Modal>
