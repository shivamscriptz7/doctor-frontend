...
// import { useState, useEffect, useMemo, useCallback } from 'react';
// import { Stethoscope, Plus, Search, Download, X, Edit, Trash2, Eye, FileText, Sheet, ChevronDown, Loader2 } from 'lucide-react';
// import { getDoctorApi, createDoctorApi, updateDoctorApi, deleteDoctorApi, getDepartmentApi,countDoctorApi } from '../../lib/commonApis';
...
// const EMPTY_FORM = {
//   name: '', hospitalName: '', qualification: '', departmentId: '',
//   email: '', phone: '', address: '', pincode: '', city: '', state: '', country: '', availability: 'true',  // ✅ यह add करो
// };
...
//     setLoading(true);
//     try {
//       const res = await getDoctorApi(100, 1);  // ✅ हमेशा page 1
//       const dataArray = res?.data?.doctors || res?.data || res || [];
//       if (!Array.isArray(dataArray)) { setDoctors([]); return; }
//       setDoctors(dataArray.map(item => ({
//         id:            item.id,
...
//         state:         item.state    || '-',
//         country:       item.country  || '-',
//         // दोनों API calls में यह line add करो
// active: item.active === 'true',  // boolean true/false जाएगा
//       })));
//     } catch (err) {
//       console.error(err);
//       showToast('❌ Failed to load doctors');
...
//       setLoading(false);
//     }
//   }, []);  // ✅ dependency array खाली
//   const loadDepartments = useCallback(async () => {
//     try {
//       const res = await getDepartmentApi(10, 1);
//       const dataArray = res?.departments || res?.data || [];
//       setDepartments(dataArray.map(item => ({ id: item.id, name: item.departmentName })));
//     } catch (err) {
//       console.error(err);
...
// const [unavailableDoctors, setUnavailableDoctors] = useState(0);
...
//       setUnavailableDoctors(data.unavailableDoctors);
//     })
//     .catch(err => console.error(err));
//     loadDoctors(); loadDepartments(); }, [loadDoctors]);
...
//         country:       formData.country,
//         // दोनों API calls में यह line add करो
// active: formData.availability === 'true',  // boolean true/false जाएगा
//       });
//       await loadDoctors();
//       setShowAddModal(false);
//       resetForm();
...
//       country:       doctor.country === '-' ? '' : doctor.country,
//       // दोनों API calls में यह line add करो
// active: doctor.active === 'true',  // boolean true/false जाएगा
//     });
//     setShowEditModal(true);
//   };
...
//         <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
//           {[
//            { label: 'Total Doctors', value: totalDoctors, color: 'text-slate-800' },
//   { label: 'Departments', value: totalDepartments, color: 'text-emerald-600' },
//   { label: 'Available Doctors', value: availableDoctors, color: 'text-teal-600' },
//   { label: 'Unavailable Doctors', value: unavailableDoctors, color: 'text-red-500' },
//           ].map(({ label, value, color }) => (
//             <div key={label} className="bg-white rounded-xl p-4 shadow-sm border border-slate-100">
//               <p className="text-sm text-slate-600 mb-1">{label}</p>
//               <p className={`text-2xl font-bold ${color}`}>{value}</p>
        ...
        <div className="absolute z-20 w-full mt-1 bg-white border border-emerald-100 rounded-xl shadow-xl overflow-hidden max-h-48 overflow-y-auto">
          ...
          {departments.map((dept) => (
            <button
              key={dept.id}
              type="button"
              onClick={() => { onChange({ target: { name: 'departmentId', value: String(dept.id) } }); setOpen(false); }}
              className={`w-full px-4 py-2.5 text-left text-sm font-medium transition-colors flex items-center gap-2 ${
                String(value) === String(dept.id)
                  ? 'bg-emerald-500 text-white'
                  : 'text-slate-700 hover:bg-emerald-50 hover:text-emerald-700'
...
function AvailabilityDropdown({ value, onChange }) {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className={selected ? 'text-slate-700' : 'text-slate-400'}>
          {selected ? selected.label : 'Select Availability'}
        </span>
        <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform duration-200 ${open ? 'rotate-180 text-emerald-600' : ''}`} />
      </button>
      {open && (
        <div className="absolute z-20 w-full mt-1 bg-white border border-emerald-100 rounded-xl shadow-xl overflow-hidden">
          {options.map((opt) => (
            <button
              key={opt.value}
              type="button"
              onClick={() => { onChange({ target: { name: 'availability', value: opt.value } }); setOpen(false); }}
              className={`w-full px-4 py-2.5 text-left text-sm font-medium transition-colors flex items-center gap-2 ${
                value === opt.value
                  ? 'bg-emerald-500 text-white'
                  : 'text-slate-700 hover:bg-emerald-50 hover:text-emerald-700'
            ...
            >
              {value === opt.value && <span className="w-2 h-2 rounded-full bg-white inline-block flex-shrink-0" />}
              {opt.label}
            </button>
          ))}
        </div>
...
function DoctorFormFields({ formData, onChange, departments }) {
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className={labelCls}>Qualification</label>
        <input type="text" name="qualification" value={formData.qualification} onChange={onChange} className={inputCls} placeholder="e.g. MBBS, MD, MS — Cardiology" />
      </div>

      {/* Row 2: Department + Availability */}
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className={labelCls}>Department Name</label>
          <DepartmentDropdown value={formData.departmentId} departments={departments} onChange={onChange} />
        ...
        <div>
          <label className={labelCls}>Availability *</label>
          <AvailabilityDropdown value={formData.availability} onChange={onChange} />
        </div>
      </div>
...
export default function DoctorsPage() {
  ...
  const loadDoctors = useCallback(async () => {
    ...
    try {
      ...
      setDoctors(dataArray.map(item => ({
        ...
        state:         item.state    || '-',
        country:       item.country  || '-',
        active:        item.active,   // ✅ boolean as-is
      })));
    } catch (err) {
      console.error(err);
      showToast('error','Failed','Failed to load doctors');
      setDoctors([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleAddDoctor = async (e) => {
    ...
    try {
     const res =  await createDoctorApi({
        ...
        state:         formData.state,
        country:       formData.country,
        active:        formData.availability === 'true',  // ✅ boolean
      });
      console.log(res,'resssssssss');
      ...
      resetForm();
      setCurrentPage(1);
      showToast('success','Created','Doctor created successfully!');
    } catch (error) {
  ...
  const handleEditClick = (doctor) => {
    ...
    setFormData({
      name:          doctor.name,
      hospitalName:  doctor.hospitalName === '-' ? '' : doctor.hospitalName,
      qualification: doctor.qualification === '-' ? '' : doctor.qualification,
      departmentId:  doctor.departmentId?.toString() || '',
      email:         doctor.email,
      phone:         doctor.phone,
      ...
      state:         doctor.state   === '-' ? '' : doctor.state,
      country:       doctor.country === '-' ? '' : doctor.country,
      availability:  doctor.active ? 'true' : 'false',  // ✅ boolean → string
    });
    setShowEditModal(true);
  };
  ...
  const handleDeleteDoctor = async () => {
    ...
    try {
      ...
      setShowDeleteModal(false);
      setSelectedDoctor(null);
      showToast('error','Deleted','Doctor deleted successfully');
  ...
  // ── Export ────────────────────────────────────────────────────────────────
  const exportToExcel = () => {
    const headers = ['ID', 'Name', 'Hospital', 'Qualification', 'Department', 'Email', 'Phone', 'City', 'State', 'Country', 'Availability'];
    const csv = [
      headers.join(','),
      ...filteredDoctors.map(d =>
        `${d.id},"${d.name}","${d.hospitalName}","${d.qualification}","${d.department}","${d.email}","${d.phone}","${d.city}","${d.state}","${d.country}","${d.active ? 'Yes' : 'No'}"`
      ),
    ].join('\n');
    const link = document.createElement('a');
    link.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8;' }));
  ...
  const exportToPDF = () => {
    const html = `<!DOCTYPE html><html><head><title>Doctors Report</title><style>
      ...
      <table>
        <thead><tr>
          <th>Name</th><th>Hospital</th><th>Qualification</th>
          <th>Department</th><th>Email</th><th>Phone</th><th>City</th><th>Availability</th>
        </tr></thead>
        <tbody>${filteredDoctors.map(d => `
          <tr>
            <td>${d.name}</td><td>${d.hospitalName}</td><td>${d.qualification}</td>
            ...
            <div key={doctor.id} className="bg-white rounded-2xl p-6 shadow-lg border border-slate-100 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
              <div className="flex items-center gap-4 mb-4">
                ...
                <span className={`flex-shrink-0 px-2.5 py-1 rounded-full text-xs font-semibold ${
                  doctor.active ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600'
                }`}>
        ...
        {selectedDoctor && (
          <div className="space-y-6">
            <div className="flex items-center gap-4 pb-6 border-b border-slate-200">
              ...
              {/* ✅ Availability badge in view modal */}
              <span className={`px-3 py-1.5 rounded-full text-sm font-semibold ${
                selectedDoctor.active ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600'
              }`}>
                {selectedDoctor.active ? '✓ Available' : '✗ Unavailable'}
              </span>
            </div>
